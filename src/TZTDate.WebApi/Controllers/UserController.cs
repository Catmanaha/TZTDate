...
public class UserController : ControllerBase {
  ...
  public async Task<IActionResult> Login(UserLoginDto userdto) {
    try {
      await sender.Send(new LoginCommand() { userLoginDto = userdto });
    } catch (Exception ex) {
      ModelState.AddModelError("Error", ex.Message);
      return NotFound();
    }
  ...
  public async Task<IActionResult> UploadAvatar(IFormFile file) {
    ...
    // User path = await context.Users.FirstOrDefaultAsync(e => e.Id ==
    // user.Id); path.ProfilePicPath = filename; await
  ...
  [HttpGet]
  [Authorize]
  public async Task<List<User>> Profiles(string? searchByName, int? startAge,
                                         int? endAge, string? interests,
                                         Gender? searchGender) {
    User me = await userManager.GetUserAsync(User);
    var users = await context.Users.ToListAsync();
    SearchData searchData =
        new SearchData() { Me = me,
                           Users = users,
    ...
    return users.GetRange(0, users.Count() < pageItemsCount ? users.Count()
                                                            : pageItemsCount);
  }

  [Authorize]
  public async Task<List<User>> LoadMoreProfiles(int skip, string? searchByName,
    ...
    users = SearchDataService.MoreProfilesFilter(new SearchData() {
      ...
      SearchingStartAge = startAge, SearchingEndAge = endAge,
      SearchingInterests = interests, SearchingUsername = searchByName
    });
    users = users.GetRange(skip, users.Count() - skip < pageItemsCount
                                     ? users.Count() - skip
                                     : pageItemsCount);
    return users;
  }
